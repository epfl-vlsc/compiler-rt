...
DECLARE_REAL(int, memcmp, const void *a1, const void *a2,
...
DECLARE_REAL(char *, strncpy, char *to, const char *from,
             __sanitizer::uptr size)
DECLARE_REAL(__sanitizer::uptr, strnlen, const char *s,
             __sanitizer::uptr maxlen)
...
#define MEMORO_INTERCEPT_FUNC_VER(name, ver)                                   \
  do {                                                                         \
    if ((!INTERCEPT_FUNCTION_VER(name, ver) || !REAL(name)))                   \
      VReport(1, "AddressSanitizer: failed to intercept '" #name "@@" #ver     \
                 "'\n");                                                       \
  } while (0)
#else
